...
- If a liquidity token is configured, reads the configured liquidity source balance through the Stellar Asset Contract interface before transferring.
- Reverts with `ContractError::InsufficientLiquidity` (15) if the configured reserve balance is lower than the requested draw amount.
- On reserve failure, borrower utilization is not updated and no transfer is attempted.
